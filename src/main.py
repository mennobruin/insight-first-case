...
def last_n_days(n: int, today: dt.date):
...
def run(config: Config) -> int:
    ...
    for date in last_n_days(n=DAYS_TO_FETCH, today=today):
        try:
            snapshot = client.fetch_historical(date)
            rows.extend(
                to_eur_rates(snapshot.rates, date, snapshot.timestamp, fetched_at)
            )
        except OxrError:
            # Transient/per-day fetch problem: skip this date, keep the rest.
            fetch_failures += 1
            logger.warning(f"Skipping {date} due to fetch error", exc_info=True)
        except MissingCurrencyError:
            # This date's payload is unusable; skip it rather than the whole run.
            data_failures += 1
            logger.warning(f"Skipping {date} due to data error", exc_info=True)
